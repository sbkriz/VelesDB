...
1. Create test file with tests for:
   ...
   - `test_near_fused_in_match_where_is_error` — NEAR_FUSED in MATCH WHERE → Error (not silently ignored)
...
**Files:** `crates/velesdb-core/src/collection/search/query/mod.rs`, `extraction.rs`, `match_exec/where_eval.rs`
...
7. **BUG-FIX (PR #119 pattern):** NEAR_FUSED in MATCH WHERE silently passes as `Ok(true)`.
   In `where_eval.rs:90`, `VectorFusedSearch` is grouped with `VectorSearch` as pass-through.
   Unlike `VectorSearch` (handled by `execute_match_with_similarity`), `VectorFusedSearch` has
   NO higher-level handler → all nodes match silently (incorrect results).
   **Fix:** Change `where_eval.rs` to return `Error::Config("NEAR_FUSED not supported in MATCH WHERE")` for `VectorFusedSearch`, keeping `VectorSearch` as pass-through.
